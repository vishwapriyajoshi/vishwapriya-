...
import pandas as pd
fcast = pd.read_csv("C:/Users/user/Downloads/plasticSales.csv")
import numpy as np
p = fcast["Month"][0]
p[0:3]
fcast['months']= 0
...
for i in range(60):
    p = fcast["Month"][i]
    fcast['months'][i]= p[0:3]
    
month_dummies = pd.DataFrame(pd.get_dummies(fcast['months']))
fcast1 = pd.concat([fcast,month_dummies],axis = 1)

fcast1["t"] = np.arange(1,61)

fcast1["t_squared"] = fcast1["t"]*fcast1["t"]
fcast1.columns
fcast1["log_Rider"] = np.log(fcast1["Sales"])
fcast1.Sales.plot()
Train = fcast1.head(47)
Test = fcast1.tail(12)
